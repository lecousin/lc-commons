...
public interface BytesDataIO extends IO {
	...
	/** @return encoding used by this IO. */
	Endianness getEndianness();
	
	/** Readable bytes data. */
	interface Readable extends BytesDataIO, IO.Readable {
		...
		/** Read all bytes of the given buffer.
		 * 
		 * @param buf buffer to fill
		 * @throws IOException in case of error
		 */
		...
		interface Seekable extends BytesDataIO.Readable, IO.Seekable {
			...
			/**
			 * Read a signed byte at the given position. 
			 * @param pos position
			 * @return value read
			 * @throws IOException in case of error
			 */
			 ...
			 * @throws IOException in case of error
			 */
			void readFullyAt(long pos, byte[] buf, int off, int len) throws IOException;
			
			/** Read all bytes of the given buffer at the given position.
			 * 
			 * @param pos position
			 * @param buf buffer to fill
			 * @throws IOException in case of error
			 */
			default void readFullyAt(long pos, byte[] buf) throws IOException {
				if (isClosed()) throw new ClosedChannelException();
				readFullyAt(pos, buf, 0, buf.length);
			}
	...
	/** Writable bytes data. */
	interface Writable extends BytesDataIO, IO.Writable {
		 ...
		 * @throws IOException in case of error
		 */
		void writeFully(byte[] buf, int off, int len) throws IOException;
		
		/** Write bytes.
		 * @param buf buffer
		 * @throws IOException in case of error
		 */
		default void writeFully(byte[] buf) throws IOException {
			if (isClosed()) throw new ClosedChannelException();
			writeFully(buf, 0, buf.length);
		}
		...
		interface Seekable extends BytesDataIO.Writable, IO.Seekable {
			...
			/** Write a signed byte, at the given position.
			 * @param pos position
			 * @param value value
			 * @throws IOException in case of error
			 */
			...
			/** Write bytes at the given position.
			 * @param pos position
			 * @param buf buffer
			 * @param off offset
			 * @param len number of bytes
			 * @throws IOException in case of error
			 */
			void writeFullyAt(long pos, byte[] buf, int off, int len) throws IOException;
			 ...
			 * @throws IOException in case of error
			 */
			default void writeFullyAt(long pos, byte[] buf) throws IOException {
				if (isClosed()) throw new ClosedChannelException();
				writeFullyAt(pos, buf, 0, buf.length);
			}
